...
- **Automatic partitioning**: Hypertables handle time-based partitioning automatically (by time AND `app_id`)
...
> **Multi-App Note:** All applications in the ecosystem connect to the **same** TimescaleDB instance. Logs are partitioned by `app_id` which is auto-injected from the `APP_ID` environment variable. See [MULTI-APP-ECOSYSTEM-ARCHITECTURE.md](./MULTI-APP-ECOSYSTEM-ARCHITECTURE.md) for the shared infrastructure setup.
...
-- This enables efficient queries filtered by app_id
...
-- Segment by app_id AND event_type for efficient per-app decompression
ALTER TABLE logs SET (
  timescaledb.compress,
  timescaledb.compress_segmentby = 'app_id, event_type',
...
```sql
-- Hourly aggregates for dashboard queries (includes app_id for multi-app ecosystem)
...
-- Daily aggregates for longer-term analytics (includes app_id for multi-app ecosystem)
...
FROM logs
GROUP BY bucket, app_id, event_type, geo_country
WITH NO DATA;
...
export type LogRecordType = {
  appId?: string                            // Auto-injected from APP_ID env var
  createdAt?: Date
  deviceId?: string
  errorCode?: string
...
export type LogsHourlySummaryType = {
  appId: string                             // Required for multi-app ecosystem
...
export type GetLogsListQueryType = {
  appId?: string                            // Filter by specific app (parent dashboard only)
  endDate?: Date
  eventType?: LogEventType
  filterValue?: string
  fingerprint?: string
  ...
  limit?: number
  offset?: number
  orderBy?: 'appId' | 'createdAt' | 'eventType' | 'ipAddress'
  sortDir?: 'ASC' | 'DESC'
  startDate?: Date
  success?: boolean
  userId?: string
...
export async function createLogsSchema(): Promise<void> {
  ...
  // Create the base table with app_id for multi-app ecosystem
...
export class LoggingService {
   ...
   * APP_ID is automatically injected from environment - cannot be overridden
   * This ensures logs are always correctly partitioned by application
   */
  private async insertLog(data: LogRecordType): Promise<void> {
    const sql = `
      INSERT INTO logs (
        app_id, event_type, event_subtype, user_id, fingerprint, device_id,
    ...
    await TimescaleDbConnection.query(sql, [
      APP_ID,  // Always inject from environment - ensures correct app_id partitioning
      data.eventType,
  ...
  async getLogsList(query: GetLogsListQueryType): Promise<GetLogsListResponseType> {
    ...
    const {
      appId,  // Only used by parent dashboard
      endDate,
      eventType,
      fingerprint,
      ipAddress,
    ...
    if (IS_PARENT_DASHBOARD_APP) {
      ...
      // If no appId filter, sees ALL apps' logs
    } else {
      // Regular apps ALWAYS filter by their own APP_ID
      conditions.push(`app_id = $${paramIndex++}`)
      params.push(APP_ID)
    }
  ...
  async getSummary(hoursAgo: number = 24): Promise<LoggingSummaryResponseType> {
    ...
    const result = await TimescaleDbConnection.query<{
      ...
      SELECT
        ...
        COUNT(DISTINCT ip_address) as unique_ips
      FROM logs
      WHERE created_at >= NOW() - INTERVAL '${safeHours} hours' ${appFilter}
    `)
