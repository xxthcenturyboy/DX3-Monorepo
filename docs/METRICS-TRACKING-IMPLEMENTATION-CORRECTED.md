...
| Join with user data | ❌ | ✅ |
| **Cross-app analytics** | Per-property | ✅ Single source |
| **App-specific filtering** | Limited | ✅ By `app_id` |
...
- ✅ Examples:
  ...
  - `GET /api/metrics/apps` - List of apps with metrics (parent dashboard only)
...
GROUP BY bucket, app_id, event_type, method, signup_source, geo_country
WITH NO DATA;
...
```json
{
  ...
  "VIEWING_ALL_APPS": "Viewing all apps",
  "VIEWING_APP": "Viewing {appName}",
...
```
┌────────────────────────────────────────────────────────────────┐
│  Metrics Dashboard    [All Apps ▼] [Last 30d ▼] [Refresh 🔄]   │
│                       ↑ App filter (parent dashboard only)     │
├────────────────────────────────────────────────────────────────┤
│                                                                │
│  ┌─────────────────────────────────────────────────────────┐  │
│  │                    USER GROWTH                           │  │
...
export const MetricsAdminDashboard = () => {
  ...
  const { data: summary, error: summaryError, isLoading: summaryLoading } =
    useGetMetricsSummaryQuery({ appId: appFilter, refreshKey })

  const { data: growth, isLoading: growthLoading } =
    useGetGrowthTrendQuery({ appId: appFilter, range: dateRange, refreshKey })
...
```sql
-- For specific app (or regular app's own data)
SELECT
  bucket::date as date,
...
-- For parent dashboard (cross-app summary)
SELECT
  app_id,
  method,
  SUM(total_count) as count
...
WHERE event_type = 'METRIC_SIGNUP'
  AND bucket >= NOW() - INTERVAL '30 days'
GROUP BY app_id, method
ORDER BY app_id, method;
```
