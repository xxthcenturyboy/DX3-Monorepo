...
import { AwaiterLottie } from './awaiter.lottie'
...
describe('AwaiterLottie', () => {
  it('should render without crashing', () => {
    const { container } = render(<AwaiterLottie />)
    expect(container.firstChild).not.toBeNull()
  })

  it('should render LottieWrapper with correct animation data', () => {
    const { getByTestId } = render(<AwaiterLottie />)
    const wrapper = getByTestId('lottie-wrapper')

    expect(wrapper).toHaveAttribute(
      'data-animation-data',
      '/assets/animations/loading-orange-diffuse.json',
    )
  })
  ...
  it('should use speed of 2.5', () => {
    const { getByTestId } = render(<AwaiterLottie />)
    const wrapper = getByTestId('lottie-wrapper')

    expect(wrapper).toHaveAttribute('data-speed', '2.5')
  })

  it('should accept complete callback', () => {
    const mockComplete = jest.fn()
    render(<AwaiterLottie complete={mockComplete} />)
