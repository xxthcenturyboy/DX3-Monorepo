...
export const FeatureFlagAdminCreateDialog: React.FC<CreateDialogPropsType> = (props) => {
  ...
  const [name, setName] = useState<FeatureFlagNameType>(FEATURE_FLAG_NAMES.BLOG)
  ...
  const [
    createFlag,
    {
      ...
      isUninitialized: createUninitialized,
      reset: resetCreate,
    },
  ...
  const reset = () => {
    ...
    setShowError(false)
    setErrorMessage('')
    setName(FEATURE_FLAG_NAMES.BLOG)
    setDescription('')
    setStatus(FEATURE_FLAG_STATUS.DISABLED)
    setTarget(FEATURE_FLAG_TARGET.ALL)
    setPercentage('')
  }
  ...
  useEffect(() => {
    if (!isCreating && !createUninitialized) {
      if (createError) {
        setErrorMessage(getErrorStringFromApiResponse(createError))
        setShowError(true)
      } else if (createSuccess) {
        setAllSucceeded(true)
      }
    }
  }, [isCreating, createError, createUninitialized, createSuccess])

  const submitDisabled = (): boolean => {
    return !name || !description || isCreating
  }
        ...
        >
          ...
          >
            ...
            <Select
              ...
              value={name}
            >
              {FEATURE_FLAG_NAMES_ARRAY.map((n) => (
                <MenuItem
                  key={n}
                  value={n}
                >
